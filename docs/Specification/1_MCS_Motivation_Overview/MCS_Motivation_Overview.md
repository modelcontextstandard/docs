...
Prompt engineering, in particular, changes from a per-project cost into a one-time investment with compounding returns. When someone improves the prompts inside a driver, every application using it benefits immediately. Combined with dynamic prompt loading ([Section 11](../11_LLM_Prompt_Patterns/LLM_Prompt_Patterns.md)), prompt configurations become versionable, tradeable artifacts.
...
This is the ecosystem MCS is designed to enable. [Section 12](../12_Ecosystem_and_Division_of_Labor.md) explores it in detail.
